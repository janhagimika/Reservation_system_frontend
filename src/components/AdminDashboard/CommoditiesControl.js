...
function FullCommoditiesControl() {
    ...
    const handleAddItem = async () => {
        ...
        try {
            const serviceResponse = await axiosConfig.post('/services', {
                serviceName: newItem.serviceName
            });

            //backend here needs to return the json

            const serviceData = await serviceResponse.data;
            const serviceId = serviceData.serviceId;
            const retrievedServiceName = serviceData.serviceName;

            const commodityResponse = await axiosConfig.post(endpoint, { ...newItem, 
                service:{ serviceId: serviceId, serviceName: retrievedServiceName }});
