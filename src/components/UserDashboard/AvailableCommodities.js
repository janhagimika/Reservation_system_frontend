...
function AvailableCommodities({ refreshReservations }) {    // Accept fetchReservations as a prop
    ...
    const handleReservation = async (commodity, type) => {
        ...
        if (selectedStartTime >= selectedEndTime) {
            alert('Čas začátku rezervace musí být před časem konce rezervace.');
            return;
        }
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                alert(`Chyba při rezervaci: ${response.data?.error || 'Neznámá chyba'}`);
            }
        } catch (error) {
            ...
            alert(`Chyba při rezervaci: ${errorMessage}`);
        }
    };
    ...
    const handleCommoditySelection = (commodity, type) => {
        ...
        if (selectedDate) { // Add this check
            fetchAvailableTimes(commodity.serviceId, type, selectedDate);
        }
    ...
    const handleDateChange = (date) => {
        ...
        if (selectedCommodity) {
            fetchAvailableTimes(selectedCommodity.serviceId, selectedCommodity.type, date);  // Fetch available times on date change
        }
    };
                ...
                <div className="reservation-form" ref={reservationFormRef}>
                    {selectedCommodity && (
                        <>
                            <h3>Rezervace</h3>
                            <div>
                                <p>Rezervujete: <strong>{selectedCommodity?.serviceName}</strong></p>
                            </div>
                            <div>
